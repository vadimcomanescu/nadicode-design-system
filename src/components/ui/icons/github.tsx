  ...
  ({ onMouseEnter, onMouseLeave, className, size = 28, ...props }, ref) => {
    ...
    const handleMouseEnter = useCallback(
      async (e: React.MouseEvent<HTMLDivElement>) => {
        if (!isControlledRef.current) { bodyControls.start("animate"); await tailControls.start("draw"); tailControls.start("wag"); }
        onMouseEnter?.(e);
      },
    ...
    const handleMouseLeave = useCallback(
      (e: React.MouseEvent<HTMLDivElement>) => {
        if (!isControlledRef.current) { bodyControls.start("normal"); tailControls.start("normal"); }
        onMouseLeave?.(e);
      },
